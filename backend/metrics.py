...
def compute_deliverable_metrics(d: dict) -> dict:
    """Compute all metrics for a single complete deliverable."""
    xcsg_pd = compute_person_days(d["xcsg_calendar_days"], d["xcsg_team_size"])
    legacy_pd = compute_person_days(d["legacy_calendar_days"], d["legacy_team_size"])
    effort_ratio = compute_effort_ratio(legacy_pd, xcsg_pd)
    quality_ratio = compute_quality_ratio(d["legacy_revision_rounds"], d["xcsg_revision_rounds"])
    ...
    return {
        ...
        "legacy_revisions": REVISION_NUMBERS.get(d["legacy_revision_rounds"], 1.0),
...
def determine_checkpoint(complete_count: int) -> int:
    """Determine current checkpoint based on complete deliverable count."""
...
def deliverables_to_next_checkpoint(complete_count: int) -> int:
    """How many more complete deliverables to reach next checkpoint."""
    if complete_count >= 20:
...
def compute_summary(complete_deliverables: list, total_deliverables: list) -> dict:
    """Compute aggregate summary metrics."""
    total = len(total_deliverables)
    complete = len(complete_deliverables)
    pending = total - complete

    if not complete_deliverables:
        return {
            ...
            "deliverables_to_next_checkpoint": deliverables_to_next_checkpoint(complete),
        }

    metrics_list = [compute_deliverable_metrics(d) for d in complete_deliverables]
...
def compute_trend_data(complete_deliverables: list) -> list:
    """Per-deliverable metrics in chronological order for trend charts."""
    return [compute_deliverable_metrics(d) for d in complete_deliverables]


def compute_scaling_gates(complete_deliverables: list, all_deliverables: list) -> list:
    """Evaluate all 6 scaling gates."""
    # Gate 1: Multi-engagement — ≥2 deliverable types
    types = set(d["deliverable_type"] for d in complete_deliverables)
    gate1_pass = len(types) >= 2
    gate1_detail = f"{len(types)} type{'s' if len(types) != 1 else ''} completed"

    # Gate 2: Time reduction — avg effort ratio > 1.3
    if complete_deliverables:
        metrics_list = [compute_deliverable_metrics(d) for d in complete_deliverables]
        ...
        gate2_pass = False
        gate2_detail = "No complete deliverables yet"

    # Gate 3: Client-invisible quality — ≥1 deliverable with 0 revision rounds
    zero_revision = sum(1 for d in complete_deliverables if d["xcsg_revision_rounds"] == "0")
    gate3_pass = zero_revision >= 1
    gate3_detail = f"{zero_revision} deliverable{'s' if zero_revision != 1 else ''} with 0 revisions"
    ...
    d2_values = [d.get("d2_knowledge_reuse", "") for d in complete_deliverables]
    ...
    gates = [
        {"id": 1, "name": "Multi-engagement", "description": "≥2 deliverable types completed", "status": "pass" if gate1_pass else "pending", "detail": gate1_detail},
        {"id": 2, "name": "Time Reduction", "description": "Average effort ratio > 1.3×", "status": "pass" if gate2_pass else "pending", "detail": gate2_detail},
        {"id": 3, "name": "Client-Invisible Quality", "description": "≥1 deliverable with 0 revision rounds", "status": "pass" if gate3_pass else "pending", "detail": gate3_detail},
